...
- `.github/workflows/drought-reports.yml` — runs daily; `run_pipeline.py` no-ops unless gridMET Drought has published a new pentad since the last successful run, commits only `state.json`, and deploys `site/` to GitHub Pages straight from the runner's filesystem.
