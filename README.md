...
- `data/reports/` — pipeline output: `raw/` (API status responses, gitignored), `extracted/` (unzipped report graphics/CSVs, per layer/date/site, gitignored), `state.json` (last successful run date — the one file here that IS committed, so the 5-day cadence check survives across Actions runs).
- `site/` — the static site. Entirely a build artifact of `data/reports/extracted/` (regenerated fresh every cycle) and gitignored except `site/assets/` (hand-authored CSS) — deployed straight to GitHub Pages via the Actions artifact upload, never committed. ~550MB for a full 333-polygon cycle, which would balloon the repo if committed every 5 days.
...
- `.github/workflows/drought-reports.yml` — runs daily, no-ops unless 5 days have passed, commits only `state.json`, and deploys `site/` to GitHub Pages straight from the runner's filesystem.
