...
Water district polygons are much larger than groundwater/irrigation ones,
and running many of them concurrently degrades Earth Engine's per-job
completion time badly (confirmed live: ~50% timeout at 15 concurrent, 0%
at 3 concurrent -- see DECISIONS.md "Water district timeouts under
concurrent load"). LAYER_MAX_IN_FLIGHT caps water_districts at 3, which
also -- as a side effect -- paces its ~121 requests slowly enough across
the resulting ~3 hour run that no artificial quota-spreading (chunking,
inter-chunk sleeps) is needed anymore to stay under Climate Engine's 200
req/hour cap; an earlier version of this file did that chunking and it's
been removed now that the real bottleneck (compute contention, not
request-rate quota) is fixed at the source.
...
RETRY_BACKOFF_S = 45
# Water districts: confirmed live (20/20 succeeded, 0 timeouts, at 3
# concurrent vs. ~50% timing out at 15) -- see DECISIONS.md "Water
# district timeouts under concurrent load".
LAYER_MAX_IN_FLIGHT = {
    "water_districts": 3,
}
...
def process_layer(layer: str, *, run_date: str, api_end_date: str, limit: int | None) -> None:
    max_in_flight = LAYER_MAX_IN_FLIGHT.get(layer, DEFAULT_MAX_IN_FLIGHT)
    run_layer(layer, limit=limit, end_date=run_date, api_end_date=api_end_date, max_in_flight=max_in_flight)
    # One automatic retry pass, after a short backoff: transient failures
    # (concurrency-limit errors, network blips) are expected at some rate
    # every run -- see DECISIONS.md "Second retry pass".
    time.sleep(RETRY_BACKOFF_S)
    run_layer(layer, limit=limit, end_date=run_date, api_end_date=api_end_date,
              retry_failed=True, max_in_flight=max_in_flight)
    # Anything still failing with one of the confirmed-fixable error
    # messages (a Climate Engine server-side bug on complex geometries,
    # gridMET's 4km grid missing tiny polygons, or the comma-in-name
    # sub_choices parsing bug) gets resubmitted via the coordinates
    # endpoint instead -- see DECISIONS.md and the fetch_reports.py module
    # docstring.
...
def main() -> None:
    ...
    for layer in LAYERS:
        print(f"--- {layer} ---")
        process_layer(layer, run_date=run_date, api_end_date=api_end_date, limit=args.limit)
